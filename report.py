...
def process_accuracies(batches_data, clfs, feature_key):
    ...
    for clf in clfs:
        acc = []
        for i in range(3):
            acc.append([batch_data[feature_key] for batch_data in batches_data
                        if batch_data['classifier'] == clf and batch_data['batch_number'] == (i+1)][0])
        acc = sorted(acc)        
...
def report(batches_data, clfs, folder, mode):
    ...
    for clf in clfs:
        ...
        for batch_number in range(number_of_batches):  # for each batch
            validation_f1_max_index = max((i for i, batch_data in enumerate(batches_data)
                                           if batch_data['classifier'] == clf and
                                           batch_data['batch_number'] == (batch_number+1)),
                                          key=lambda index: batches_data[index]['validation_f1'])
        ...
        selected_features = [feature for batch_data in batches_data
                             for feature in batch_data['selected_features']
                             if batch_data['classifier'] == clf]
...
def run(folder, classifiers, classifier_indices, dataset_type, num_of_packets_to_process):
    # Get the list of files generated by the optimization algorithms
    ...
    full_paths = [os.path.join(folder, file)
                  for file in file_names if file.__contains__(f"{dataset_type}_dataset_results_num_{num_of_packets_to_process}_")]

    # Get the list of optimization algorithms used to generate the files
    modes = list(set([os.path.basename(path).split('_')[6] for path in full_paths]))

    for mode in modes:  # for each optimization algorithm
        batches_data = []  # list of dictionaries containing the data of each batch
        for path in full_paths:  # for each file
            filename = os.path.basename(path)  # get the file name
            if filename.split('_')[6] == mode:  # if the file was generated by the current optimization algorithm
                with open(path, 'r') as f:  # open the file
                    content = f.readlines()  # read the file content
                
                # Get the validation accuracy
                validation_f1 = float(next(line.split()[-1] for line in content if "Best Solution:" in line))
                # Get the selected features
                selected_features = [line.strip()
                                     for line in takewhile(lambda x: x.strip(),
                                                           content[next(i for i, line in enumerate(content)
                                                                        if "Selected features:" in line) + 1:])]
                # Get the F1 values
                f1_values = [float(line.split()[-1]) for line in content if "F1" in line]

                # Add the data to the list of dictionaries
                batches_data.append({
                    "mode": filename.split('_')[6],
                    "classifier": int(filename.split('_')[8]),
                    "batch_number": int(filename.split('_')[10]),
                    "run_number": int(filename.split('_')[12].split(".")[0]),
        ...
        if classifier_indices == "":  # if no classifier indices were specified by the user
            # Get the list of classifiers used to generate the files
            clfs = sorted(list(set(batch_data['classifier'] for batch_data in batches_data)))
        else:  # if classifier indices were specified by the user
            # Get the list of classifiers used to generate the files
            clfs = list(map(int, classifier_indices.split(',')))
            for clf in clfs:  # for each classifier index
                # Check if the classifier index is valid
        ...
        plot(batches_data, clfs, folder, classifiers, mode)  # plot the diagram

        print("generating the report...")
        report(batches_data, clfs, folder, mode)  # generate the report