...
def load_csv(file_path):
    try:
        with open(file_path, 'r') as f:  # Open the CSV file for reading
            ...
            # Read the header as string and the rest as float
            data = [next(reader)] + [[float(token) for token in row] for row in reader]
        return data
...
def remove_duplicates_list_list(list_of_lists):
    # Convert all sublists except the first one to tuples (which are hashable)
    unique_sublists = set(tuple(sublist) for sublist in list_of_lists[1:])
    # Combine the first sublist with the unique sublists back into a result list
    result = [list_of_lists[0]] + [list(sublist) for sublist in unique_sublists]
...
def extract_features_and_labels(data, label_column_index):
    # Extract features by removing the label column from each row
...
def classify_after_filtering(solution, fitness_function_file_paths, test_file_path,
    ...
    f1_score_average, predictions, test_labels = classify(train, test, classifier_index, classifiers)
